/**
 * @module @createlumina/world
 */
import { deserialize } from '@createlumina/nbt';
import { FileSystem, openFileSystem } from '@createlumina/system';
    ...
    public async getAdvancementsData(): Promise<AdvancementDataFrame[]> {
        ...
        return Promise.all(
            files
                ...
                .map((f) =>
                    this.fs.readFile(this.fs.join('advancements', f)).then((b) => deserialize<AdvancementDataFrame>(b)),
                ),
        );
    }
}
