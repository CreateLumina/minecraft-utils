...
} from '@createlumina/core';
import {
    ChecksumNotMatchError,
    ChecksumValidatorOptions,
    DownloadBaseOptions,
    JsonValidator,
    Validator,
} from '@createlumina/file-transfer';
import { task, Task } from '@createlumina/task';
...
function resolveDownloadUrls<T>(
    original: string,
    version: T,
...
export type Options = DownloadBaseOptions &
    ParallelTaskOptions &
    AssetsOptions &
    JarOption &
    LibraryOptions &
...
export async function installResolvedLibraries(
    ...
    await installLibrariesTask(
        { libraries, minecraftDirectory: typeof minecraft === 'string' ? minecraft : minecraft.root },
        option,
    ).startAndWait();
}
 ...
 * @param minecraft minecraft location
 */
export function installVersionTask(
    versionMeta: MinecraftVersionBaseInfo,
    minecraft: MinecraftLocation,
    options: JarOption = {},
): Task<ResolvedVersion> {
...
export function installAssetsTask(version: ResolvedVersion, options: AssetsOptions = {}): Task<ResolvedVersion> {
    return task('assets', async function () {
        ...
        await this.all(
            objectArray.map((o) => new InstallAssetTask(o, folder, options)),
            {
                throwErrorImmediately: options.throwErrorImmediately ?? false,
                getErrorMessage: (errs) =>
                    `Errors during install Minecraft ${version.id}'s assets at ${version.minecraftDirectory}: ${errs.map(errorToString).join('\n')}`,
            },
        );
...
export function installLibrariesTask(version: InstallLibraryVersion, options: LibraryOptions = {}): Task<void> {
    return task('libraries', async function () {
        const folder = MinecraftFolder.from(version.minecraftDirectory);
        await this.all(
            version.libraries.map((lib) => new InstallLibraryTask(lib, folder, options)),
            {
                throwErrorImmediately: options.throwErrorImmediately ?? false,
                getErrorMessage: (errs) =>
                    `Errors during install libraries at ${version.minecraftDirectory}: ${errs.map(errorToString).join('\n')}`,
            },
        );
    });
}
 ...
 * @param option The install option
 */
export function installResolvedLibrariesTask(
    libraries: ResolvedLibrary[],
    minecraft: MinecraftLocation,
    option?: LibraryOptions,
) {
    return installLibrariesTask(
        { libraries, minecraftDirectory: typeof minecraft === 'string' ? minecraft : minecraft.root },
        option,
    );
}
...
export function installResolvedAssetsTask(assets: AssetInfo[], folder: MinecraftFolder, options: AssetsOptions = {}) {
    return task('assets', async function () {
        ...
        await this.all(
            assets.map((o) => new InstallAssetTask(o, folder, options)),
            {
...
export class InstallJsonTask extends DownloadTask {
    constructor(version: MinecraftVersionBaseInfo, minecraft: MinecraftLocation, options: Options) {
        ...
        super({
            ...
            headers: options.headers,
            agent: options.agent,
            validator: expectSha1
                ? options.checksumValidatorResolver?.({ algorithm: 'sha1', hash: expectSha1 }) || {
                      algorithm: 'sha1',
                      hash: expectSha1,
                  }
                : new JsonValidator(),
...
export class InstallJarTask extends DownloadTask {
    constructor(
        version: ResolvedVersion & { downloads: Required<ResolvedVersion>['downloads'] },
        minecraft: MinecraftLocation,
        options: Options,
    ) {
        const folder = MinecraftFolder.from(minecraft);
        ...
        super({
            url: urls,
            validator: options.checksumValidatorResolver?.({ algorithm: 'sha1', hash: expectSha1 }) || {
                algorithm: 'sha1',
                hash: expectSha1,
            },
            destination,
            ...
            validator: options.prevalidSizeOnly
                ? {
                      async validate(destination, url) {
                          ...
                          if (fstat.size !== size) {
                              throw new ChecksumNotMatchError(
                                  'size',
                                  size.toString(),
                                  fstat.size.toString(),
                                  destination,
...
export function resolveLibraryDownloadUrls(library: ResolvedLibrary, libraryOptions: LibraryOptions): string[] {
    ...
    return [
        ...new Set([
            ...
            ...normalizeArray(libraryOptions.mavenHost).map((m) =>
                joinUrl(m, url.pathname).replace('/maven/maven', '/maven'),
            ),
