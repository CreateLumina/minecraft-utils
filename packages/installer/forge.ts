...
import { parse as parseForge } from '@createlumina/forge-site-parser';
import { Task, task } from '@createlumina/task';
import { filterEntries, open, openEntryReadStream, readAllEntries, readEntry } from '@createlumina/unzip';
    ...
    constructor(
        ...
        super({
            ...
            validator: installer?.sha1
                ? options.checksumValidatorResolver?.({ algorithm: 'sha1', hash: installer?.sha1 }) || {
                      algorithm: 'sha1',
                      hash: installer?.sha1,
                  }
                : new ZipValidator(),
...
export function installByInstallerTask(
    ...
    return task('installForge', async function () {
        ...
        const jarPath = await this.yield(
            new DownloadForgeInstallerTask(forgeVersion, version.installer, mc, options, isLegacy).map(function () {
 ...
 * @throws {@link BadForgeInstallerJarError}
 */
export function installForgeTask(
    version: RequiredVersion,
    minecraft: MinecraftLocation,
    options: InstallForgeOptions = {},
): Task<string> {
