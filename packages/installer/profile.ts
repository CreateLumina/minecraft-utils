import { LibraryInfo, MinecraftFolder, MinecraftLocation, Version as VersionJson } from '@createlumina/core';
import { AbortableTask, CancelledError, task } from '@createlumina/task';
import { open, readEntry, walkEntriesGenerator } from '@createlumina/unzip';
 ...
 * @throws {@link PostProcessError}
 */
export function installByProfile(
    installProfile: InstallProfile,
 ...
 * @param options The options to install
 */
export function installByProfileTask(
    installProfile: InstallProfile,
    minecraft: MinecraftLocation,
    options: InstallProfileOption = {},
) {
    return task('installByProfile', async function () {
        ...
        const versionJson: VersionJson = await readFile(minecraftFolder.getVersionJson(installProfile.version))
            .then((b) => b.toString())
            .then(JSON.parse);
        const installRequiredLibs = VersionJson.resolveLibraries(installProfile.libraries);

        await this.all(
            installRequiredLibs.map((lib) => new InstallLibraryTask(lib, minecraftFolder, options)),
            {
                throwErrorImmediately: options.throwErrorImmediately ?? false,
                getErrorMessage: (errs) =>
                    `Errors during install libraries at ${minecraftFolder.root}: ${errs.map(errorToString).join('\n')}`,
            },
        );
        ...
        await this.all(
            libraries.map((lib) => new InstallLibraryTask(lib, minecraftFolder, options)),
            {
                throwErrorImmediately: options.throwErrorImmediately ?? false,
                getErrorMessage: (errs) =>
                    `Errors during install libraries at ${minecraftFolder.root}: ${errs.map(errorToString).join('\n')}`,
...
export class PostProcessingTask extends AbortableTask<void> {
    ...
    constructor(
        ...
        private minecraft: MinecraftFolder,
        private java: SpawnJavaOptions,
    ) {
        super();
    ...
    protected async postProcess(mc: MinecraftFolder, proc: PostProcessor, javaOptions: SpawnJavaOptions) {
        ...
        if (proc.outputs && (await this.isInvalid(proc.outputs))) {
            throw new PostProcessFailedError(
                proc.jar,
                [javaOptions.java ?? 'java', ...cmd],
                'Validate the output of process failed!',
            );
        }
    }
