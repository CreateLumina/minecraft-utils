...
import { ChecksumValidatorOptions, DownloadBaseOptions, DownloadOptions, Validator } from '@createlumina/file-transfer';
...
export async function fetchJavaRuntimeManifest(
    ...
    if (!manifestIndex) {
        const response = await request(normalizeUrls(options.url ?? DEFAULT_RUNTIME_ALL_URL, options.apiHost)[0], {
        ...
        await this.all(
            Object.entries(manifest.files)
                ...
                .map(([file, entry]) => {
                    ...
                    const downloadInfo =
                        downloadLzma && fEntry.downloads.lzma ? fEntry.downloads.lzma : fEntry.downloads.raw;
                    ...
                    const downloadOptions: DownloadOptions = {
                        url: urls,
                        validator: options.checksumValidatorResolver?.({ algorithm: 'sha1', hash }) || {
                            algorithm: 'sha1',
                            hash,
                        },
                        destination: dest,
