import { getPlatform, Platform } from '@createlumina/core';
...
import { DownloadBaseOptions } from '@createlumina/file-transfer';
...
export function installJreFromMojangTask(options: InstallJavaOptions) {
    const { destination, unpackLZMA, cacheDir = tmpdir(), platform = getPlatform() } = options;
    return task('installJreFromMojang', async function () {
        const info: { [system: string]: { [arch: string]: { jre: DownloadInfo } } } = await this.yield(
            task('fetchInfo', async () => {
                const response = await request('https://launchermeta.mojang.com/mc/launcher.json', {
        ...
        const lzmaPath = await this.yield(
            new DownloadJRETask(info[system][currentArch].jre, cacheDir, options).map(function () {
                return this.to!;
            }),
        );
...
export async function getPotentialJavaLocations(): Promise<string[]> {
    ...
    if (currentPlatform === 'win32') {
        ...
        unchecked.add(
            `C:\\Program Files (x86)\\Minecraft Launcher\\runtime\\jre-legacy\\windows-${currentArch}\\jre-legacy`,
        );
