import { MinecraftFolder, MinecraftLocation, Version as VersionJson } from '@createlumina/core';
import { open, readEntry } from '@createlumina/unzip';
...
export async function installNeoForged(
    project: 'forge' | 'neoforge',
    version: string,
...
export function installNeoForgedTask(
    project: 'forge' | 'neoforge',
    version: string,
    minecraft: MinecraftLocation,
    options: InstallForgeOptions,
    ...
    return task('installForge', async function () {
        ...
        const jarPath = await this.yield(
            new DownloadNeoForgedInstallerTask(project, version, mc, options).map(function () {
