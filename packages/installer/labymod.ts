import { MinecraftFolder, MinecraftLocation, LibraryInfo } from '@createlumina/core';
import { DownloadBaseOptions } from '@createlumina/file-transfer';
import { Task, task, AbortableTask, CancelledError } from '@createlumina/task';
...
class JsonTask extends AbortableTask<string> {
    ...
    constructor(
        private manifest: LabyModManifest,
        private tag: string,
        private folder: MinecraftFolder,
    ...
    protected async process(): Promise<string> {
        ...
        const libraries: LibInfo[] = await request(librariesUrl, {
        ...
        const versionJson = await request(versionInfo.customManifestUrl, {
            dispatcher: this.dispatcher,
            signal: this.controller.signal,
        }).then((res) => res.body.json() as any);
...
export function installLabyMod4Task(
