import { MinecraftFolder, MinecraftLocation } from '@createlumina/core';
...
export namespace LiteloaderVersionList {
    export function parse(content: string) {
        ...
        for (const mcversion in result.versions) {
            const versions: { release?: LiteloaderVersion; snapshot?: LiteloaderVersion } = ((metalist.versions as any)[
                mcversion
            ] = {});
            ...
            if (snapshots) {
                const { stream, file, version, md5, timestamp, tweakClass, libraries } =
            ...
            if (artifacts) {
                const { stream, file, version, md5, timestamp, tweakClass, libraries } =
                    artifacts['com.mumfrey:liteloader'].latest;
                const type = stream === 'RELEASE' ? 'RELEASE' : 'SNAPSHOT';
                versions.release = {
                    url,
                    type,
 ...
 */
export async function getLiteloaderVersionList(
    options: {
        /**
 ...
 */
export function installLiteloader(
 ...
 * @param version The real existed version id (under the the provided minecraft location) you want to installed liteloader inherit
 */
export function installLiteloaderTask(
    versionMeta: LiteloaderVersion,
    location: MinecraftLocation,
    options: InstallOptions = {},
): Task<string> {
    return task('installLiteloader', async function installLiteloader() {
        ...
        const mountedJSON: any = await this.yield(
            task('resolveVersionJson', async function resolveVersionJson() {
                ...
                return readFile(mc.getVersionJson(mountVersion))
                    .then((b) => b.toString())
                    .then(JSON.parse);
            }),
        );

        const versionInf = await this.yield(
            task('generateLiteloaderJson', async function generateLiteloaderJson() {
