import { diagnose, MinecraftFolder, MinecraftLocation, Version } from '@createlumina/core';
...
describe.skip('ForgeInstaller', () => {
    test('should install forge on 1.7.10', async ({ temp }) => {
        ...
        await expect(
            existsSync(
                join(temp, 'versions', '1.7.10-Forge10.13.4.1614-1.7.10', '1.7.10-Forge10.13.4.1614-1.7.10.json'),
            ),
        ).resolves.toBeTruthy();
        const resolvedVersion = await Version.parse(temp, result);
        // https://github.com/CreateLumina/minecraft-utils/issues/210
        resolvedVersion.libraries
            .filter((lib) => lib.groupId === 'org.scala-lang.plugins')
            .forEach((lib) => {
                (lib.download as any).sha1 = '';
            });
    ...
    test('should install forge 1.12.2-14.23.5.2852', async ({ temp }) => {
        ...
        await expect(
            existsSync(join(temp, 'versions', '1.12.2-forge-14.23.5.2852', '1.12.2-forge-14.23.5.2852.json')),
        ).resolves.toBeTruthy();
    ...
    test('should install forge 1.14.4-forge-28.0.45', async ({ temp }) => {
        ...
        await expect(
            existsSync(join(temp, 'versions', '1.14.4-forge-28.0.45', '1.14.4-forge-28.0.45.json')),
        ).resolves.toBeTruthy();
        await installDependencies(await Version.parse(temp, result));
    });
}, 100000000);
...
describe.skip('LiteloaderInstaller', () => {
    ...
    describe('#install', () => {
        ...
        test('should be able to install liteloader to forge', async ({ temp }) => {
            ...
            const result = await installLiteloader(meta, MinecraftFolder.from(temp), {
                inheritsFrom: '1.12.2-forge-14.23.5.2852',
            });
            const resolvedVersion = await Version.parse(temp, result);
            // https://github.com/CreateLumina/minecraft-utils/issues/210
            resolvedVersion.libraries
