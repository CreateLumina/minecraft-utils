import { ClassReader, ClassVisitor, Opcodes } from '@createlumina/asm';
import { MinecraftFolder, MinecraftLocation, Version } from '@createlumina/core';
import { task } from '@createlumina/task';
import { getEntriesRecord, open, readAllEntries, readEntry } from '@createlumina/unzip';
...
export function installOptifineTask(
    ...
    return task('installOptifine', async function () {
        ...
        const entry =
            record['net/optifine/Config.class'] ?? record['Config.class'] ?? record['notch/net/optifine/Config.class'];
        if (!entry) {
