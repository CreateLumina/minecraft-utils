...
export async function diagnose(
    ...
    try {
        ...
        if (e.error === 'CorruptedVersionJson') {
            issues.push({
                type: 'corrupted',
                role: 'versionJson',
                file: minecraft.getVersionJson(e.version),
                expectedChecksum: '',
                receivedChecksum: '',
                hint: 'Re-install the minecraft!',
            });
        } else {
            issues.push({
                type: 'missing',
    ...
    if (!assetIndexIssue) {
        const objects = (
            await readFile(minecraft.getAssetsIndex(resolvedVersion.assets), 'utf-8').then((b) =>
...
export async function diagnoseLibraries(
    ...
    minecraft: MinecraftFolder,
    options?: DiagnoseOptions,
): Promise<Array<LibraryIssue>> {
    const signal = options?.signal;
    const issues = await Promise.all(
        resolvedVersion.libraries.map(async (lib) => {
...
export async function diagnoseAssetIndex(
    resolvedVersion: ResolvedVersion,
    minecraft: MinecraftFolder,
    ...
    const issue = await diagnoseFile({
        ...
        expectedChecksum: resolvedVersion.assetIndex?.sha1 ?? '',
        role: 'assetIndex',
        hint: 'Problem on assets index file! Please consider to use Installer.installAssets to fix.',
    });
    if (issue) {
...
export async function diagnoseJar(
    resolvedVersion: ResolvedVersion,
    minecraft: MinecraftFolder,
    options?: DiagnoseOptions & { side?: 'client' | 'server' },
): Promise<MinecraftJarIssue | undefined> {
    const side = options?.side ?? 'client';
    const jarPath = minecraft.getVersionJar(resolvedVersion.minecraftVersion, side);
    const issue = await diagnoseFile({
        file: jarPath,
        expectedChecksum: resolvedVersion.downloads[side]?.sha1 ?? '',
        role: 'minecraftJar',
        hint: 'Problem on Minecraft jar! Please consider to use Installer.instalVersion to fix.',
    });
    if (issue) {
        return Object.assign(issue, { version: resolvedVersion.minecraftVersion });
    }
