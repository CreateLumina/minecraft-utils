...
export type VersionParseError =
    | ((BadVersionJsonError | CorruptedVersionJsonError | MissingVersionJsonError | CircularDependenciesError) & Error)
    | Error;
    ...
    export function checkAllowed(rules: Rule[], platform: Platform = getPlatform(), features: string[] = []): boolean {
        ...
        for (const rule of rules) {
            ...
            if ('os' in rule && rule.os) {
                ...
                if (platform.name === osRule.name && (!osRule.version || platform.version.match(osRule.version))) {
                    apply = true;
                }
            }
            if (apply) {
