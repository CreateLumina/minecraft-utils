import { open, openEntryReadStream, walkEntriesGenerator } from '@createlumina/unzip';
...
export async function launch(options: LaunchOption): Promise<ChildProcess> {
    ...
    const version =
        typeof options.version === 'string' ? await Version.parse(resourcePath, options.version) : options.version;
...
export async function generateArgumentsServer(options: MinecraftServerOptions | ServerOptions) {
    ...
    const cmd = [javaPath, `-Xms${minMemory}M`, `-Xmx${maxMemory}M`, ...extraJVMArgs];
    if ('path' in options) {
