...
export class YggdrasilClient {
    ...
    async login(
        ...
        if (response.status >= 400) {
            ...
            throw new YggdrasilError(
                ...
                response.headers.get('content-type')?.startsWith('application/json') ? JSON.parse(body) : undefined,
            );
        }
    ...
    async refresh(
        { accessToken, requestUser, clientToken }: { accessToken: string; clientToken: string; requestUser?: boolean },
        signal?: AbortSignal,
    ) {
