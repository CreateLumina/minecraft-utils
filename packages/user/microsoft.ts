...
export class MicrosoftAuthenticator {
    ...
    async authorizeXboxLive(
        ...
        relyingParty: 'rp://api.minecraftservices.com/' | 'http://xboxlive.com' = 'rp://api.minecraftservices.com/',
        signal?: AbortSignal,
    ) {
        ...
        if (xstsResponse.status !== 200) {
            throw new Error(
                `Failed to authorize with xbox live, status code: ${xstsResponse.status}: ${await xstsResponse.text()}}`,
            );
        }
