...
export class DownloadAgent {
    ...
    async dispatch(
        ...
        destination: string,
        handle: FileHandle,
        progressController: ProgressController | undefined,
        ...
        if (errors.length > 0) {
            if (policy && ranges.length > 0) {
                await this.checkpointHandler
                    ?.put(url, handle, destination, {
