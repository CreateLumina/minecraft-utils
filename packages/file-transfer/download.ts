...
export async function download(options: DownloadOptions) {
    ...
    try {
        ...
        try {
            ...
            if (pendingFile) {
                ...
                await rename(pendingFile, destination).catch((e) => {
                    throw new DownloadFileSystemError(
                        'Download file destination already existed',
                        urls,
                        headers,
                        destination,
                        e,
        ...
        throw new DownloadAggregateError(
            e,
            'Multiple errors occurred during download process',
            urls,
            headers,
            destination,
        );
