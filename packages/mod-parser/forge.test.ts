...
describe('Forge', () => {
    test('should identify the package usage', async ({ mock }) => {
        const { usedForgePackage, usedLegacyFMLPackage, usedMinecraftClientPackage, usedMinecraftPackage } =
            ...
            modsToml: [
                {
                    ...
                    credits: '',
                    dependencies: [
                        { mandatory: true, modId: 'forge', ordering: 'NONE', side: 'BOTH', versionRange: '[28.1.0,)' },
                    ],
                    description:
                        'JEI is an item and recipe viewing mod for Minecraft, built from the ground up for stability and performance.\n',
                    displayName: 'Just Enough Items',
    ...
    test('should read @Mod in jar', async ({ mock }) => {
        ...
        expect(modAnnotations.some((m) => m.modid === 'NuclearCraft' && m.version === '1.9e')).toBeTruthy();
    });
