...
describe('ModrinthV2Client', () => {
    ...
    describe('#getProjectVersions', () => {
        ...
        it('should be able to get project versions with options', async () => {
            ...
            const client = new ModrinthV2Client({ dispatcher: agent });
            await expect(
                client.getProjectVersions('123', { loaders: ['forge'], gameVersions: ['1.22'], featured: true }),
            ).resolves.toEqual({ data: 'hello' });
        });
    });
    ...
    describe('#getProjectVersionsById', () => {
        it('should be able to get project versions by id', async () => {
            ...
            mockPool
                ...
                .reply(200, { data: 'hello' });
            const client = new ModrinthV2Client({ dispatcher: agent });
            await expect(client.getProjectVersionsById(['123', '345'])).resolves.toEqual({ data: 'hello' });
        });
        it('should throw error if project not found', async () => {
            const mockPool = agent.get('https://api.modrinth.com');
            mockPool
    ...
    describe('#getProjectVersionsByHash', () => {
        it('should be able to get project versions by hash', async () => {
            ...
            mockPool
                ...
                .reply(200, { data: 'hello' });
            const client = new ModrinthV2Client({ dispatcher: agent });
            await expect(client.getProjectVersionsByHash(['123', '345'])).resolves.toEqual({ data: 'hello' });
        });
        it('should throw error if project not found', async () => {
            const mockPool = agent.get('https://api.modrinth.com');
            mockPool
                ...
                .reply(404, { data: 'hello' });
            const client = new ModrinthV2Client({ dispatcher: agent });
            await expect(client.getProjectVersionsByHash(['123', '345'])).rejects.toThrow(ModerinthApiError);
        });
        it('should be able to set algorithm', async () => {
            ...
            mockPool
                ...
                .reply(200, { data: 'hello' });
            const client = new ModrinthV2Client({ dispatcher: agent });
            await expect(client.getProjectVersionsByHash(['123', '345'], 'sha256')).resolves.toEqual({ data: 'hello' });
        });
    });
