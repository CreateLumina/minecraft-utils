...
describe('Resourcepack', () => {
    ...
    describe('#head', () => {
        ...
        test('should throw if there is no pack.meta in directory', async () => {
            ...
            await expect(readPackMetaAndIcon(`${root}/resourcepacks/empty-resourcepack`)).rejects.toThrowError(
                new Error('Illegal Resourcepack: Cannot find pack.mcmeta!'),
            );
        });
        test('should throw if there is no pack.meta in zip', async () => {
            await expect(readPackMetaAndIcon(`${root}/resourcepacks/empty-resourcepack.zip`)).rejects.toThrowError(
                new Error('Illegal Resourcepack: Cannot find pack.mcmeta!'),
            );
        });
    });
