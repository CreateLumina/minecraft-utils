import { BlockModel, PackMeta } from '@createlumina/resourcepack';
...
export class BlockModelObject extends Object3D {
    ...
    getCenter() {
        ...
        return new Vector3((box.minx + box.maxx) / 2, (box.miny + box.maxy) / 2, (box.minz + box.maxz) / 2);
    }
}
    ...
    getObject(model: BlockModel.Resolved, options: { uvlock?: boolean; y?: number; x?: number } = {}, fix = 0.001) {
        ...
        for (const element of model.elements) {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                let map = [new Vector2(x1, y2), new Vector2(x2, y2), new Vector2(x1, y1), new Vector2(x2, y1)];
